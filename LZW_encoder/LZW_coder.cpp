...
// генераци€ файла на основе алфавита
...
void LZW_encoder(const std::vector<std::string>& init_dict, const std::string& fin_name, const std::string& fout_name) {
	...
	while (fin >> std::noskipws >> symbol) {
		...
		if (std::find(dictionary.begin(), dictionary.end(), str) == dictionary.end()) {
			...
			// следующа€ строка начинаетс€ с последнего символа новой дл€ словар€ строки
			str = eChar;
		}
	}
...
void LZW_decoder(const std::vector<std::string>& init_dict, const std::string& fin_name, const std::string& fout_name) {
	...
	while (!fin.eof()) {
		...
		// если строки ещЄ нет в словаре, то дополн€ем еЄ самосто€тельно (ссылка на объ€снение)
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (std::find(dictionary.begin(), dictionary.end(), str) == dictionary.end() and flag) {
				...
				// следующа€ строка начинаетс€ с невошедших символов tmp
				str = tmp.substr(i, tmp.size() - i); //tmp.size() - i символов начина€ с i-го 
				flag = false;
			}
			if (!flag) break;
		}
...
int getIndex(const std::vector<std::string>& dictionary, const std::string& str) {
	...
	if (it != dictionary.end()) {
		...
		std::cout << "Ёлемент " << str << " не найден в векторе." << std::endl;
	}
}

// число в 10-чной ——, полученное из его двоичного представлени€ 
...
// »щет и возвращает ближайшую к x сверху степень двойки
...
// бинарное представление числа с разр€дностью x, где 2^x >= dict_size
